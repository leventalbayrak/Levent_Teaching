...
int main(int argc, char **argv)
{
	...
	int playground_x = screen_width / 2 - screen_height / 8;
	int playground_y = screen_height / 2 - screen_height / 8;
	int playground_w = screen_height / 4;
	int playground_h = screen_height / 4;
	
	int target_size = screen_height / 20;
	int bullseye_size = target_size / 4;
	int target_position_x = playground_x + (playground_w - target_size) * rand() / RAND_MAX;
	int target_position_y = playground_y + (playground_h - target_size) * rand() / RAND_MAX;
		...
		if (game_started == 1)
		{
			if (mouse_clicked == 1)
			{
				...
				if (check_Hit(current_mouse_x, current_mouse_y, target_position_x, target_position_y, target_size, target_size) == 1)
				{
					...
					target_position_x = playground_x + (playground_w - target_size) * rand() / RAND_MAX;
					target_position_y = playground_y + (playground_h - target_size) * rand() / RAND_MAX;
