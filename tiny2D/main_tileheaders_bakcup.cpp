...
int main(int argc, char **argv)
{
	...
	Grid_Camera::Grid_Camera camera = { 50,50,128,96 };
	...
	while (!done)
	{
		...
		*/
		...
		float tw = (float)Game::screen_width / (1 + active_range.x1 - active_range.x0);
		float th = (float)Game::screen_height / (1 + active_range.y1 - active_range.y0);
		
		float ty = ((float)active_range.y0 - camera.y) * th;
		for (int i = active_range.y0; i <= active_range.y1; i++)
		{
			float tx = ((float)active_range.x0 - camera.x) * tw;
			for (int j = active_range.x0; j <= active_range.x1; j++)
			{
				int grid_data = level.data[i*level.n_cols + j];
