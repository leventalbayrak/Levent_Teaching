...
namespace My_Game
{
	...
	void update_Actor(Actor *p, unsigned int current_time)
	{
		...
		int on_ground = 0;
		Collision::Point_Feeler object_feelers;
		Collision::point_Feeler_Pos(&object_feelers, &p->world_coord, 0.1, 0.2);
		
		if (Grid::tile(&object_feelers.bottom_feeler, &World::collision_map) > 0 ||
			Grid::tile(&object_feelers.bottomleft_feeler, &World::collision_map) > 0 ||
			Grid::tile(&object_feelers.bottomright_feeler, &World::collision_map) > 0)
		{
			on_ground = 1;
		}
		...
		Shape::Rect tmp_coord;
		Collision::Point_Feeler collision_feelers;
		
		tmp_coord.x = World::bodies.pos[p->physics_body].x;
		tmp_coord.y = World::bodies.pos[p->physics_body].y;
		tmp_coord.w = p->world_coord.w;
		tmp_coord.h = p->world_coord.h;

		tmp_coord.x += World::bodies.vel[p->physics_body].x;

		Collision::point_Feeler_Pos(&collision_feelers, &tmp_coord, 0.05, 0.1);

		if (Grid::tile(&collision_feelers.left_feeler, &World::collision_map) > 0 ||
			Grid::tile(&collision_feelers.right_feeler, &World::collision_map) > 0)
		{
			World::bodies.vel[p->physics_body].x = 0.0;
			printf("collision X\n");
		}
		else
		{
			World::bodies.pos[p->physics_body].x = tmp_coord.x;
		}

		tmp_coord.x = World::bodies.pos[p->physics_body].x;
		tmp_coord.y = World::bodies.pos[p->physics_body].y;

		tmp_coord.y += World::bodies.vel[p->physics_body].y;

		Collision::point_Feeler_Pos(&collision_feelers, &tmp_coord, 0.05, 0.1);

		if (Grid::tile(&collision_feelers.top_feeler, &World::collision_map) > 0 ||
			Grid::tile(&collision_feelers.bottom_feeler, &World::collision_map) > 0)
		{
			World::bodies.vel[p->physics_body].y = 0.0;
			printf("collision Y\n");
		}
		else
		{
			World::bodies.pos[p->physics_body].y = tmp_coord.y;
		}
