...
namespace WR
{
	...
	void destroy(Data *d)
	{
		for (int i = 0; i < d->n_words; i++) free(d->words[i]);
		free(d->words);
		*d = {};
	}
	...
	void less(const Data *data, int count)
	{
		if (count > data->n_words) count = data->n_words;
	...
	void tail(const Data *data, int count)
	{
		if (count > data->n_words) count = data->n_words;

		for (int i = data->n_words - count; i < data->n_words; i++)
		{
			printf("%s\n", data->words[i]);
		}
	}

	void random(const Data *data, int count)
	{
	
		for (int i = 0; i < count; i++)
		{
			int k = (data->n_words-1) * (double)rand() / RAND_MAX;
			printf("%s\n", data->words[k]);
		}
	}
}
