...
int main(int argc, char* args[])
{
	...
	int jelly_count = 0;
	Jelly::Jelly jelly;
	Jelly::init(&jelly, 100, 40);

	int floor_size = 10;
	Jelly::Jelly floor;
	Jelly::init(&floor, screen_w*2/floor_size+screen_h*2/floor_size, floor_size);
	int k = 0;
	
	for (int x = 0; x < screen_w; x += floor_size)
	{	
		floor.pos_t[k].x = x;
		floor.pos_t[k].y = 0;
		floor.pos_t_minus_1[k] = floor.pos_t[k];
		k++;
	}
	for (int x = 0; x < screen_w; x += floor_size)
	{
		floor.pos_t[k].x = x;
		floor.pos_t[k].y = screen_h - floor_size;
		floor.pos_t_minus_1[k] = floor.pos_t[k];
		k++;
	}
	for (int y = 0; y < screen_h; y += floor_size)
	{
		floor.pos_t[k].x = 0;
		floor.pos_t[k].y = y;
		floor.pos_t_minus_1[k] = floor.pos_t[k];
	...
	for (int y = 0; y < screen_h; y += floor_size)
	{
		floor.pos_t[k].x = screen_w - floor_size;
		floor.pos_t[k].y = y;
		floor.pos_t_minus_1[k] = floor.pos_t[k];
		k++;
	}
	...
	for (;;)
	{
		...
		while (SDL_PollEvent(&e) != 0)
		{
			...
			else if (e.type == SDL_FINGERUP)
			{
				...
				jelly.pos_t[jelly_count] = { mx, my };
				jelly.pos_t_minus_1[jelly_count].x = jelly.pos_t[jelly_count].x + 4.0 * (1.0 - 2.0 * rand() / RAND_MAX);
				jelly.pos_t_minus_1[jelly_count].y = jelly.pos_t[jelly_count].y + 4.0 * (1.0 - 2.0 * rand() / RAND_MAX);
				jelly_count++;
		...
		for (int i = 0; i < jelly.n_points; i++)
		{
			jelly.accel[i].y += 600.0;
		}
		...
		Jelly::draw(&floor, &camera_pos, NULL);
